...
const GrowthChart = ({ analytics }) => {
      ...
      <div className="erp-card" style={{ padding: '1.25rem 1.4rem', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <div style={{ width: '28px', height: '28px', borderRadius: '6px', background: 'var(--brand-blue-light)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--brand-blue)' }}>
              <TrendingUp size={15} />
            </div>
          ...
          <span style={{ fontSize: '0.72rem', padding: '2px 8px', borderRadius: '4px', background: 'var(--bg-subtle)', border: '1px solid var(--border-color)', color: 'var(--text-secondary)' }}>
            6-Month Trend
          </span>
        </div>
  ...
  return (
    <div className="erp-card" style={{ padding: '1.25rem 1.4rem', height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ width: '100%', height: '240px', minHeight: '240px' }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={formattedData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            ...
            <Tooltip
              contentStyle={{ background: 'var(--bg-card)', border: '1px solid var(--border-color)', borderRadius: '8px', color: 'var(--text-primary)', fontSize: '0.85rem' }}
            />
      ...
      {latestInsight && (
        <div style={{ marginTop: '1rem', padding: '0.85rem 1rem', background: 'var(--bg-subtle)', borderRadius: '8px', borderLeft: '3px solid var(--accent-purple)', fontSize: '0.85rem', color: 'var(--text-secondary)' }}>
          <strong style={{ color: 'var(--text-primary)' }}>AI Growth Insight:</strong> {latestInsight}
        </div>
      )}
    </div>
  );
