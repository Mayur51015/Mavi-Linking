...
const SkillRadar = () => {
  ...
  if (loading) {
    return (
      ...
      >
        <Loader2 size={24} className="animate-spin" style={{ color: 'var(--brand-blue)' }} />
        <div style={{ color: 'var(--text-secondary)', fontSize: '0.8125rem' }}>Analyzing skill profile...</div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <span style={{ fontSize: '0.72rem', padding: '3px 9px', borderRadius: '4px', background: 'var(--bg-subtle)', border: '1px solid var(--border-color)', color: 'var(--text-secondary)' }}>
          Confidence Index: <strong style={{ color: 'var(--accent-cyan, #06B6D4)' }}>{confidence}%</strong>
        </span>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(220px, 1fr) minmax(240px, 1.25fr)', gap: '1.25rem', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
          <div>
            ...
            <div style={{ fontSize: '1.15rem', fontWeight: 800, color: 'var(--brand-blue)', fontFamily: 'Inter, sans-serif', marginTop: '0.1rem' }}>
              {specialization}
            </div>
          </div>

          {/* Key Strengths */}
          <div style={{ background: 'var(--bg-subtle)', border: '1px solid var(--border-color)', borderRadius: '6px', padding: '0.5rem 0.65rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.72rem', fontWeight: 600, color: 'var(--accent-emerald, #22C55E)', marginBottom: '0.2rem' }}>
              <Trophy size={11} /> Key Strengths
            </div>
            <ul style={{ paddingLeft: '1rem', margin: 0, fontSize: '0.75rem', color: 'var(--text-secondary)', lineHeight: 1.35 }}>
              {strengths.map((s, i) => (
                <li key={i}>{s}</li>
              ))}
            </ul>
          ...
          {/* Areas to Improve */}
          <div style={{ background: 'var(--bg-subtle)', border: '1px solid var(--border-color)', borderRadius: '6px', padding: '0.5rem 0.65rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', fontSize: '0.72rem', fontWeight: 600, color: 'var(--accent-red, #EF4444)', marginBottom: '0.2rem' }}>
          ...
          {/* Core Technologies */}
          <div>
            <div style={{ fontSize: '0.68rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.04em', fontWeight: 600, marginBottom: '0.3rem' }}>
              Core Technologies
            </div>
