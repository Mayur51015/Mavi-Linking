  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: 'var(--bg-primary)' }}>
      <div className="glass-card animate-fade-in" style={{ width: '100%', maxWidth: '580px', padding: '2.5rem', borderRadius: '16px', background: 'var(--bg-card)', border: '1px solid var(--border-color)', boxShadow: 'var(--shadow-xl)' }}>
            ...
            <div key={s.num} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', opacity: step >= s.num ? 1 : 0.4 }}>
              <div
                style={{
                  ...
                  height: '28px',
                  borderRadius: '50%',
                  background: step > s.num ? 'var(--accent-emerald, #22c55e)' : step === s.num ? 'var(--brand-blue)' : 'var(--border-color)',
                  color: 'white',
                  display: 'flex',
                  alignItems: 'center',
        ...
        {/* Error Alert Banner */}
        {error && (
          <div style={{ background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.3)', color: 'var(--accent-red)', padding: '0.85rem 1rem', borderRadius: '8px', marginBottom: '1.5rem', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            <AlertCircle size={18} />
            <span>{error}</span>
          </div>
        )}
        ...
        {step === 1 && (
          <form onSubmit={handleValidateInstitutionCode}>
            <div style={{ marginBottom: '1.5rem', textAlign: 'center' }}>
              <div style={{ width: '56px', height: '56px', borderRadius: '50%', background: 'var(--brand-blue-light)', border: '1px solid rgba(59, 130, 246, 0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1rem' }}>
                <Building2 size={28} color="var(--brand-blue)" />
              </div>
              <h3 style={{ fontSize: '1.25rem', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '0.25rem' }}>Join Your Institution</h3>
              <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem' }}>Enter the unique Institution Code provided by your college/university.</p>
            </div>

            <div style={{ marginBottom: '1.5rem' }}>
              <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '0.5rem' }}>Institution Code</label>
              <input
                type="text"
                ...
                required
                style={{ width: '100%', padding: '0.75rem 1rem', borderRadius: '8px', background: 'var(--bg-input)', border: '1px solid var(--border-color)', color: 'var(--text-primary)', fontSize: '1rem', letterSpacing: '1px', textTransform: 'uppercase', fontWeight: 700 }}
              />
            </div>
        ...
        {step === 2 && validatedInstitution && (
          <form onSubmit={handleValidatePRN}>
            ...
            <div style={{ background: 'var(--bg-subtle)', border: '1px solid var(--border-color)', borderRadius: '10px', padding: '1rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <span style={{ fontSize: '0.75rem', color: 'var(--accent-emerald, #4ade80)', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                  <CheckCircle2 size={14} /> Confirmed Institution
                </span>
                <strong style={{ color: 'var(--text-primary)', display: 'block', fontSize: '0.95rem', marginTop: '0.2rem' }}>{validatedInstitution.name}</strong>
                <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Code: {validatedInstitution.code}</span>
              </div>
              <button
                type="button"
                onClick={handleChangeInstitution}
                style={{ background: 'transparent', border: '1px solid var(--border-color)', color: 'var(--text-secondary)', padding: '0.35rem 0.65rem', borderRadius: '6px', fontSize: '0.75rem', cursor: 'pointer' }}
              >
                Change Code
              </button>
            </div>
            ...
            <div style={{ marginBottom: '1.25rem' }}>
              <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '0.5rem' }}>Academic Department</label>
              {loadingDepartments ? (
                <div style={{ color: 'var(--text-muted)', fontSize: '0.85rem' }}>Loading institution departments...</div>
              ) : (
                <select
                  value={selectedDepartmentId}
                  onChange={(e) => setSelectedDepartmentId(e.target.value)}
                  required
                  style={{ width: '100%', padding: '0.75rem 1rem', borderRadius: '8px', background: 'var(--bg-input)', border: '1px solid var(--border-color)', color: 'var(--text-primary)', fontSize: '0.9rem' }}
                >
                  {departments.map((dept) => (
                    <option key={dept.id || dept.departmentId} value={dept.id || dept.departmentId} style={{ background: 'var(--bg-card)', color: 'var(--text-primary)' }}>
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '0.5rem' }}>Permanent Registration Number (PRN / Roll No.)</label>
              <input
                type="text"
                ...
                required
                style={{ width: '100%', padding: '0.75rem 1rem', borderRadius: '8px', background: 'var(--bg-input)', border: '1px solid var(--border-color)', color: 'var(--text-primary)', fontSize: '0.95rem' }}
              />
            </div>
            ...
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              <button
                type="button"
                onClick={() => setStep(1)}
                style={{ width: '35%', padding: '0.75rem', borderRadius: '8px', background: 'transparent', border: '1px solid var(--border-color)', color: 'var(--text-secondary)', fontWeight: 600, cursor: 'pointer' }}
              >
                Back
              </button>
              <button
        ...
        {step === 3 && prnVerified && (
          <form onSubmit={handleRegisterSubmit}>
            <div style={{ background: 'var(--bg-subtle)', border: '1px solid var(--border-color)', borderRadius: '10px', padding: '0.85rem', marginBottom: '1.25rem', fontSize: '0.8rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <span style={{ color: 'var(--accent-emerald, #4ade80)', fontWeight: 600 }}>✓ Verified: {validatedInstitution.name}</span>
                <span style={{ color: 'var(--text-muted)', display: 'block' }}>PRN: {prnInput}</span>
              </div>
              <button type="button" onClick={() => setStep(2)} style={{ background: 'transparent', border: 'none', color: 'var(--brand-blue)', textDecoration: 'underline', fontSize: '0.75rem', cursor: 'pointer' }}>Edit</button>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '0.4rem' }}>Full Name</label>
                <input
                  type="text"
                  ...
                  required
                  style={{ width: '100%', padding: '0.65rem 0.85rem', borderRadius: '8px', background: 'var(--bg-input)', border: '1px solid var(--border-color)', color: 'var(--text-primary)', fontSize: '0.85rem' }}
                />
              </div>

              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '0.4rem' }}>Email Address</label>
                <input
                  type="email"
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1rem' }}>
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '0.4rem' }}>Password</label>
                <PasswordInput
                  placeholder="••••••••"
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '0.4rem' }}>Confirm Password</label>
                <PasswordInput
                  placeholder="••••••••"
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '0.4rem' }}>Degree Program</label>
                <select
                  value={formData.degree}
                  onChange={(e) => updateField('degree', e.target.value)}
                  style={{ width: '100%', padding: '0.65rem 0.85rem', borderRadius: '8px', background: 'var(--bg-input)', border: '1px solid var(--border-color)', color: 'var(--text-primary)', fontSize: '0.85rem' }}
                >
                  <option value="B.Tech" style={{ background: 'var(--bg-card)', color: 'var(--text-primary)' }}>B.Tech</option>
                  <option value="M.Tech" style={{ background: 'var(--bg-card)', color: 'var(--text-primary)' }}>M.Tech</option>
                  <option value="B.E." style={{ background: 'var(--bg-card)', color: 'var(--text-primary)' }}>B.E.</option>
                  <option value="BCA" style={{ background: 'var(--bg-card)', color: 'var(--text-primary)' }}>BCA</option>
                  <option value="MCA" style={{ background: 'var(--bg-card)', color: 'var(--text-primary)' }}>MCA</option>
                  <option value="B.Sc" style={{ background: 'var(--bg-card)', color: 'var(--text-primary)' }}>B.Sc</option>
                </select>
              </div>

              <div>
                <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 600, color: 'var(--text-primary)', marginBottom: '0.4rem' }}>Graduation Year</label>
                <input
                  type="text"
                  placeholder="2026"
